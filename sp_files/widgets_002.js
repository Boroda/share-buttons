 ...
 * from Adobe Systems Incorporated.
 */
CQ.analytics.TargetPanel = CQ.Ext.extend(CQ.Ext.Panel, {
    
    campaignType: null,
    conversionFieldSet: null,
    conversionGoalName : null,
    conversionStyleName: null,
    conversionStyleClickThrough: null,
    
    constructor: function(config) {
        var dlg = this;

        this.campaignType = new CQ.form.Selection({
            type: 'select',
            name: './campaignType',
            fieldLabel: CQ.I18n.getMessage('Campaign type'),
            fieldDescription: CQ.I18n.getMessage("Choose 'Landing Page' to target experiences to segments, use 'AB..N' to test experiences. Defaults to 'AB..n'."),
            options: [
                  { text: '&nbsp;', value: ''},
                  { text: CQ.I18n.getMessage('AB...N'), value: 'abn'},
                  { text: CQ.I18n.getMessage('Landing page'), value: 'landingPage'}
            ]
        });
        
        var campaignTypeFieldSet = new CQ.form.DialogFieldSet({
            collapsed: false,
            autoHeight: true,
            autoScroll: true,
            xtype: 'dialogfieldset',
        ...
        this.conversionGoalName = new CQ.Ext.form.TextField({
        ...
        this.conversionStyleClickThrough = new CQ.Ext.form.Radio({
            checked: true,
            fieldLabel : CQ.I18n.getMessage("Click-through"),
            name: './conversionStyle',
            inputValue: 'clickthrough',
            listeners: {
                check: function(cmp, checked) {
                    dlg.reconcileComponentEnablements(cmp);
                }
            }
        });
        
        this.conversionStyleName = new CQ.Ext.form.Radio({
            fieldLabel : CQ.I18n.getMessage("Conversion Name/ID"),
            name: './conversionStyle',
            inputValue: 'name',
            listeners: {
                check: function(cmp, checked) {
        ...
        this.conversionFieldSet = new CQ.form.DialogFieldSet({
            ...
            xtype: 'dialogfieldset',
            items: [
               this.conversionStyleClickThrough,
               this.conversionStyleName,
               this.conversionGoalName
            ]
        });
        
        CQ.Util.applyDefaults(config, {
            "items": [
                campaignTypeFieldSet,
                dlg.conversionFieldSet
            ],
            listeners: {
                beforeshow: function(cmp) {
                    dlg.reconcileComponentEnablements();
                }
            }
        });
        
        CQ.analytics.TargetPanel.superclass.constructor.call(this, config);
    },
    
    initComponent: function() {
        CQ.analytics.TargetPanel.superclass.initComponent.call(this);

        var currentObj = this;
        // TODO react to changes in the cloud service ?
        this.on("render", function() {
            this.parentDialog = this.findParentByType("dialog");
            if (this.parentDialog) {
                this.parentDialog.on("loadcontent", function(e) {
                    var cloudConfigPanels = this.find('xtype', 'cloudservicepanel');
                    var hasTestandTargetConfig = false;
                    if ( cloudConfigPanels.length == 1 ) {
                        var cloudConfigPanel = cloudConfigPanels[0];
                        var attachedConfigurations = cloudConfigPanel.data["cq:cloudserviceconfigs"]; 
                        if ( typeof attachedConfigurations == 'string' ) {
                            var cloudConfigPanelurationData = CQ.HTTP.eval(CQ.shared.HTTP.externalize("/etc/cloudservices.infinity.json"));

                            hasTestandTargetConfig = currentObj.hasCloudConfiguration(attachedConfigurations, "testandtarget", cloudConfigPanelurationData);
                        } else if ( attachedConfigurations ) {
                            
                            var cloudConfigPanelConfigurationData = CQ.HTTP.eval(CQ.shared.HTTP.externalize("/etc/cloudservices.infinity.json"));
                            
                            for ( var i = 0 ; i < attachedConfigurations.length; i ++ ) {

                                hasTestandTargetConfig = currentObj.hasCloudConfiguration(attachedConfigurations[i], "testandtarget", cloudConfigPanelConfigurationData);
                                if ( hasTestandTargetConfig )
                                    break;
                            }
                        }
                    }

                    currentObj.reconcileComponentEnablements();
    ...
    hasCloudConfiguration: function(configurationPath, cloudServiceName, cloudConfigurationData) {
        
        // deleting cloud configurations can lead to empty paths in the cloudservicepanel
        if ( configurationPath.indexOf("/etc/cloudservices/") !== 0 )
            return false;
        
        var cloudConfigShortPath = configurationPath.substring("/etc/cloudservices/".length);
        var cloudConfigPathSegments = cloudConfigShortPath.split('/');
        var data = cloudConfigurationData;
        while ( cloudConfigPathSegments.length !== 0 ) {
            var segment = cloudConfigPathSegments.shift();
            data = data[segment];
        }

        var cloudConfigProperties = data["jcr:content"];
        
        return cloudConfigProperties["cq:cloudservicename"] == cloudServiceName;
    },
    
    /**
     * Ensures that the conversionGoalName enabled state is in sync with the conversion style value
     */
    reconcileComponentEnablements: function(cmp) {
      
        if ( this.conversionStyleName.getValue() ) {
            this.conversionGoalName.enable();
            this.conversionGoalName.validate();
        } else {
            this.conversionGoalName.disable();
            this.conversionGoalName.clearInvalid();
        }
    }
});
CQ.Ext.reg("targetpanel", CQ.analytics.TargetPanel);
