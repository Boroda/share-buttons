 ...
 *
 * [
 *   { "name": "email", "type": "string", "key": "osrEmail" },
 *   { "name": "postal code", "type": "string", "key": "osrPostalCode" },
 *   { "name": "consent checkbox", "type": "boolean", "key": "osrConsent" },
 * ]
 *
 * @businessRequirements @json
 *
 * [
 *   { "name": "email acquisition", "desc": "client expects email acquisition with email and postal code captured" },
 *   { "name": "impression", "desc": "basket still has to be captured in status1, make sure to push to __scd.b.i" },
 *   { "name": "multiple sites", "desc": "they actually want this on two sites, have to two double client config hack" }
 * ]
 *
 */
...
__sco.config = {
  ...
  'guid': '440973f2-a6ca-4cc1-b378-2b482840448c', // V2 guid
  'v1guid': '440973f2-a6ca-4cc1-b378-2b482840448c', // V1 guid, the api key for the client

  'triggers': ['exit', 'timeout'], // Set the triggers to use, use either load or exit and timeout together

  'status1': [function() {
    var pages = [
          { url: 'localhost', continue: true },
          { url: 'categories', continue: true },
          { url: 'store-locator', continue: true }
        ],
        i = 0,
        sportchek = 'https://www.sportcheck.ca/',
        atmosphere = 'https://www.atmosphere.ca/'

    if (__sco.loc === sportchek || __sco.loc === atmosphere) {
      return false;
    }

    for (i; i < pages.length; i++) {
      if (__sco.contains(__sco.loc, pages[i].url) && typeof __scd === 'object') {
        __scd.t = 100;
        return pages[i].continue;
      }
    }

    return false;
  }], // Strings to be set in lower case, will be compared against the URL of the current page. Functions will be executed and must return either true, false or a number to use as the status
  'status2': ["STATUSTWO"],
  'status3': ["/checkout/confirmation"],
  'status4': ["STATUSFOUR"],
  'exclude': ["RUN ON EVERYTHING"],

  'onchange': {
    'email': ['input.footer-section__input:first', '#signin input.signin-form__email:first', 'footer input[name="email"]:first', '.shipping-step__container input.label-set__text-field:nth-child(6)'],
    'first': ['.shipping-step__container input.label-set__text-field:first'],
    'last': ['.shipping-step__container input.label-set__text-field:nth-child(1)'],
    'telephone': ['.shipping-step__container input.label-set__text-field:last']
  },
  'block': {},
...
__sco.scraper.statusone = function() {
 ...
 try {
     //as this is email acq osr, use dummy basket info
        __scd.b.v = '1.00';
    }
    catch (be2) {
    ...
    /*
  try {
    __scd.b.v = __sco.inbetween('Estimated Total:', 'Merchandise', _scs('.co-order-summary:first', '', ['text']), 'ff');
  ...
  try {
    __sco.each(_scs('.sc-product__top'), function(ix, val) {
      var link = _scs([val, 'a.sc-product__title-link:first']);
      // high res img
      var imgLink = 'https://fgl.scene7.com/is/image/FGLSportsLtd/%id%?bgColor=0,0,0,0&fmt=png-alpha&hei=520&resMode=sharp2&op_sharpen=1';
      __scd.b.i.push({
        'n': __sco.text(link),
        'i': __sco.attr(link, 'href').split('#')[1].split('=')[0],
        'q': _scs([val, 'input.sc-product__qty:first'], '', ['getvt']),
        'v': _scs([val, 'span.sc-product__price:first'], '', ['text', 'pricecurr']),
        'f1': __sco.attr(link, 'href'),
        'u': imgLink.replace('%id%', __sco.attr(_scs([val, 'img:first']), 'src').split('?')[0].split('/').pop())
      });
    });
  } catch (be1) {
    be1.description = "101 " + (be1.description || "");
...
__sco.scraper.statusthree = function() {
  try {
    ...
    */
    __scd.s.ordernumber = location.href.split('orderId')[1].split('=')[1];
  } catch (s3) {
...
__sco.scraper.statusfour = function() {
  try {
    __sco.on('click', function(e) {
      __scd.t = 400;
    }, _scs('a.button.button-paypal:first'));
  } catch (s4) {
        ...
        if (__sco.management.canexec()) {
            ...
            **/
            ...
            try {
              ...
              //sportchek config by default
              if (__sco.contains(__sco.loc, 'atmosphere.ca')) {
                // march 22, set live
                __sco.config.live = true;
                __sco.config.doc.i1 = 18906;
                __sco.config.doc.i = 1627;
                __sco.config.timestamptemplate.i1 = 18906;
                __sco.config.timestamptemplate.i = 1627;
                __sco.config.v1guid = 'b1ca4803-71e0-41e9-a765-230e7eb49068';
                __sco.config.guid = 'b1ca4803-71e0-41e9-a765-230e7eb49068';
              }
            }
            catch(ce) {
...
// https://github.com/SaleCycle/client_scripts/blob/master/_templates/_modules/toggle.js
!function(t,e,n,s){"use strict";"object"==typeof module&&module.exports?module.exports=s():t[e]?t[e][n]=s():t[n]=s()}(this,"__sco","toggle",function(){"use strict";function t(){function t(t){return new RegExp(t+"\\"+i+"[a-zA-Z0-9-_]+")}function n(t){return t.split(i)[0]+i+c}var s=this.el,i=this.delim,r=this.config,u=typeof r,o=arguments,c="string"===u?o[0]||e:o[1],l=t("string"===u?r:r[o[0]]);return s.className=s.className.replace(l,n),this}var e="null";return function(e,n,s){return{delim:n,config:s,el:e,set:t,unset:t}}});// https://github.com/SaleCycle/client_scripts/blob/master/_templates/_modules/toggleClass.js

    __sco.osr.clientConfigModule = function () {

      //notes => https://eu3.salesforce.com/002w000000L6j7u

      var filters = [
          {
            rx: /basketball|net|nba/i,
            filter: 'basketball'
          },
          {
            rx: /run|running|jog|jogging|cardio/i,
            filter: 'running'
          },
          {
            rx: /lift|weight|fitness|!run|!running|!jog|!jogging|!cardio/i,
            filter: 'weights'
          },
          {
            rx: /ski|alpine|skiing|!winter|!cold/i,
            filter: 'skiing'
          },
          {
            rx: /hockey|puck|!winter|nhl/i,
            filter: 'hockey'
          },
          {
            rx: /winter|snowboard|snowsports|snow|cold|!nhl/i,
            filter: 'snowsports'
          }
      ];

      meld.around(__sco.osr, "preShowChecks", function (methodCall) {
        return __sco.config.status1[0]();
      });

      meld.afterReturning(__sco.osr, "renderContent", function(){

        var i = 0,
            test,
            modal = __sco.toggle(_scs('#poly-wrap-sc-div'), ':', 'sc-filter');

        for (i; i < filters.length; i++) {
          test = filters[i].rx.test(window.location.href);
          if (test) {
            __sco.currFilter = filters[i];
            return modal.set(filters[i].filter);
          }
        }

        if (!__sco.currFilter) {
          __sco.currFilter = { filter: 'training' };
          return modal.set('training');
        }

      });

      meld.around(__sco.osr, "emailBasketClick", function (methodCall) {
        var emailAddress = __sco.osr.scrapeEmailAddress(false),
            submit = _scs('div#wrapper-sc a.send-sc:first'),
            osrEmail = __sco.toggle( _scs('div#wrapper-sc input.emb-sc:first'), ':', 'sc-validation-state' ),
            osrPostalCode = __sco.toggle( _scs('div#wrapper-sc input.postal-field-sc:first'), ':', 'sc-validation-state' ),
            osrConsent = __sco.toggle( _scs('div#wrapper-sc input.check-sc:first'), ':', 'sc-validation-state' ),
            validationSpan = _scs('div#wrapper-sc span#emailvalidation h5:first'),
            err = {
              required: 'Please correct required fields.',
              noOptIn: 'You must opt in to receive emails.'
            },
            rx = {
              email: /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
              canadaPostal: /[A-Za-z][0-9][A-Za-z]\-{0,}\s{0,}[0-9][A-Za-z][0-9]/
            },
            isValid = {
              optIn: false,
              email: false,
        ...
        // 1st guard: if consent is false do not proceed
        if (osrConsent.el.checked === false) {
          osrConsent.set('invalid');
          validationSpan.textContent = err.noOptIn;
          return false;
        } else {
          isValid.optIn = true;
          __scd.s.osrConsent = true;
          osrConsent.set('default');
        }

        // 2nd guard: if email field is empty do not proceed
        if (emailAddress.value === false) {
          osrEmail.set('invalid');
          validationSpan.textContent = err.required;
          return false;
        }

        // test for valid email
        if (rx.email.test(osrEmail.el.value)) {
          __scd.s.osrEmail = osrEmail.el.value;
          isValid.email = true;
          osrEmail.set('valid');
        } else {
          osrEmail.set('invalid');
        ...
        // test for valid canadian postal
        if (rx.canadaPostal.test(osrPostalCode.el.value)) {
          __scd.s.osrPostalCode = osrPostalCode.el.value;
          isValid.postal = true;
          osrPostalCode.set('valid');
        } else {
          osrPostalCode.set('invalid');
          validationSpan.textContent = err.required;
        }

        // 3rd guard: if validations have been fulfilled proceed
        if (isValid.optIn === true && isValid.email === true && isValid.postal === true) {
     ...
     * @scrapeEmailAddress
     *
     * @param toggle {boolean} - if false, basic alert is disabled
     * @returns {object}
     *   el {dom element} - element
     *   value {string} - element value
     */

    __sco.osr.scrapeEmailAddress = function (toggle) {
        var emailField, emailAddress, obj;
        ...
        if (emailField === null || emailField.length === 0) {
          throw "Unable to retrieve email field, selector is " + __sco.osr.activePageConfig.selectors.emailCaptureField;
        }

        emailAddress = emailField.value.trim();
        obj = { el: emailField, value: emailAddress };

        if (emailAddress.length === 0 && toggle !== false) {
    ...
    __sco.osr.show = function (force) {
        ...
        if (typeof (__sco.osr.activeInstanceConfig.autofillEmailAddress) !== "undefined") {
            var autofillConfig = __sco.osr.activeInstanceConfig.autofillEmailAddress.toLowerCase(),
                emailField = _scs(__sco.osr.activePageConfig.selectors.emailCaptureField);
