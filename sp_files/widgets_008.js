...
/**
 * Fill the dropdown with possible template changes based on templateGroups
 */
CQ.media.Publishing.templateHelper = function(path) {
    "use strict";
    ...
    //alert(url);
    // [    {        value: "pink",        text: "Pink",        qtip: "Real Pink" }, {        value: "blue",        text: "Blue",        qtip: "Real Blue" }];
    var response = CQ.HTTP.get(url);
    var options = $CQ.parseJSON(response.body);
    //alert(options);

    return options;
}

CQ.media.Publishing.paginationSelectionChange = function(selectCmp, value, checked) {
    CQ.media.Publishing.updateSmoothScrollingValue(selectCmp, checked ? "never" : "always");
};

CQ.media.Publishing.updateSmoothScrollingValue = function(component, value) {
    var dialog = component.findParentByType("dialog");
    if (typeof dialog != undefined) {
        var smoothScrollingCmpArray = dialog.find("name", "./dps-smoothScrolling");

        if (smoothScrollingCmpArray &&
            smoothScrollingCmpArray.length &&
            smoothScrollingCmpArray.length > 0) {
            var smoothScrollingCmp = smoothScrollingCmpArray[0];
            smoothScrollingCmp.setValue(value);
        }
    }
};
